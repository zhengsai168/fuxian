...
f = codecs.open('news2016zh_valid.json','r',encoding='utf-8')
lines = f.readlines()
if os.path.exists('seq2seq_config.json'):
    ...
    for s in tqdm(lines):
        js = json.loads(s)
        for w in js['content']: # 纯文本，不用分词
            chars[w] = chars.get(w,0) + 1
        for w in js['title']: # 纯文本，不用分词
            chars[w] = chars.get(w,0) + 1
...
# (batch_size, padding_len, 1)  , padding部分为0， 原句子部分为1
...
x_one_hot = Lambda(to_one_hot)([x, x_mask])  # (batch_size, 1, len(char)+4 ) 每篇文章的出现过的词id为1
x_prior = ScaleShift()(x_one_hot) # 学习输出的先验分布（标题的字词很可能在文章出现过）  
# (batch_size, 1, len(char)+4 ) 对出现过的词的向量进行缩放平移

embedding = Embedding(len(chars)+4, char_size)
x = embedding(x)   # (batch_size, padding_len, char_size)
y = embedding(y)   # (batch_size, padding_len, char_size)

# encoder，双层双向LSTM
x = Bidirectional(LSTM(int(char_size/2), return_sequences=True))(x)  
x = Bidirectional(LSTM(int(char_size/2), return_sequences=True))(x)  # (batch_size, padding_len, char_size)

# decoder，双层单向LSTM
y = LSTM(char_size, return_sequences=True)(y)
y = LSTM(char_size, return_sequences=True)(y)  # (batch_size, padding_len, char_size)
...
class Interact(Layer):
    ...
    def call(self, inputs):
        q, v, v_mask = inputs
        k = v
        mv = K.max(v - (1. - v_mask) * 1e10, axis=1, keepdims=True) # maxpooling1d  (batch_size, 1, char_size)
        mv = mv + K.zeros_like(q[:,:,:1]) # 将mv重复至“q的timesteps”份  # (batch_size, padding_len_y, char_size)
        # 下面几步只是实现了一个乘性attention
        qw = K.dot(q, self.kernel)     # (batch_size, padding_len_y, char_size)
        a = K.batch_dot(qw, k, [2, 2]) / 10.  # (batch_size, padding_len_y, padding_len_x)
...
xy = Activation('softmax')(xy)   # (batch_size, padding_len_y, len(char_size)+4 )
...
def gen_title(s, topk=3):
    ...
    for i in range(50): # 强制要求标题不超过50字
        proba = model.predict([xid, yid])[:, i, 3:] # 直接忽略<padding>、<unk>、<start>  # (3, len(char)+1 )
        log_proba = np.log(proba + 1e-6) # 取对数，方便计算
        arg_topk = log_proba.argsort(axis=1)[:,-topk:] # 每一项选出topk  # (3, 3)
        _yid = [] # 暂存的候选目标序列
        _scores = [] # 暂存的候选目标序列得分
        if i == 0:
            for j in range(topk):
